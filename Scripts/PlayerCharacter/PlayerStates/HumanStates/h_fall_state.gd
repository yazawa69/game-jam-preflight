...
@export var double_jump_state: State # transition implemented
...
var already_double_jumped: bool = false
...
func process_input(event: InputEvent) -> State:
	_input_dir = Input.get_vector("move_left", "move_right", "move_forward", "move_back")
	if Input.is_action_just_pressed("jump") and not already_double_jumped:
